...
\end{equation}
wobei $\vec{A}$ das magnetische Vektorpotential ist, welches im späterem Abschnitt \eqref{maxwell:definitionVektorpot} genauer erläutert ist.
...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\node[EcolEP,scale=0.8] at (1.95,0) {$\phi$};
	\end{tikzpicture}
...
Für diesen Raum möchten wir mittels der Varitaionsrechnung eine Gleichung entwickeln, die beschreibt, wie sich das Potentialfeld verhält. 
...
\end{pmatrix}
=
\frac{1}{2}\,\varepsilon_0\left(\phi_x^2 + \phi_y^2 + \phi_z^2\right).
...
\begin{equation}
	...
	=
	\iiint_V \underbrace{
		\frac{1}{2}\,\varepsilon_0\left(\phi_x^2 + \phi_y^2 + \phi_z^2\right)}_{\displaystyle=L(x,y,z,\phi,\phi_x,\phi_y,\phi_z)}\, dV.
...
\[
\frac{1}{2}\,\varepsilon_0\,\biggl(\underbrace{\frac{\partial}{\partial\phi}\left(\phi_x^2 + \phi_y^2 + \phi_z^2\right)}_{\displaystyle=0} - \frac{\partial}{\partial x}\frac{\partial}{\partial \phi_x}\left(\phi_x^2 + \phi_y^2 + \phi_z^2\right) - 
\frac{\partial}{\partial y}\frac{\partial}{\partial \phi_y}\left(\phi_x^2 + \phi_y^2 + \phi_z^2\right) - 
\frac{\partial}{\partial z}\frac{\partial}{\partial \phi_z}\left(\phi_x^2 + \phi_y^2 + \phi_z^2\right)\biggr)
=
0.
\]
Man sieht, dass die partielle Ableitung nach $\phi$ verschwindet.
Nach den partiellen Ableitungen nach $\phi_x$, $\phi_y$ und $\phi_z$ wird die Differentialgleichung zu
\[
\frac{1}{2}\,\varepsilon_0\left(-\frac{\partial}{\partial x}2\phi_x - \frac{\partial}{\partial y}2\phi_y - \frac{\partial}{\partial z}2\phi_z\right)
=
0.
\]
Wenn man nun noch die letzten partiellen Ableitungen macht, wird die Differentialgleichung
\begin{equation}
	- \varepsilon_0\underbrace{\left(\frac{\partial^2\phi}{\partial x^2} + \frac{\partial^2\phi}{\partial y^2} + \frac{\partial^2\phi}{\partial z^2}\right)}_{\displaystyle=0}
	=
	0.
...
Zusätzlich wird nun ersichtlich, dass der Klammerterm nach Definition \eqref{???} mit dem Laplace-Operator angewendet auf das elektrische Potentialfeld $\phi$ ersetzt werden kann.
Somit wird unsere Schlussdifferentialgleichung
\begin{equation}
	\Delta\phi
	=
	0.
...
0
\]
erfüllt, führt zu einem Gradientenfeld $\nabla\phi$, das rotationsfrei und quellenfrei ist.
