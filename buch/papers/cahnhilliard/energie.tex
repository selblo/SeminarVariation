...
&=
N_V \int_\Omega \left[
  F(c) + \underbrace{\left( \frac{\kappa_2}{2} - \pderiv{\kappa_1}{c} \right)}_{\frac{1}{2}\varepsilon^2} \abs{\nabla c}^2  + \ldots
\right]\di{x}
\nonumber
\\
&=
N_V \int_\Omega \left[
  F(c) + \frac{\varepsilon^2}{2} \abs{\nabla c}^2  + \ldots
\right]\di{x}
,
\label{cahnhilliard:energy}
\end{align}
wobei $\varepsilon$ in der Praxis häufig als konstant angenommen wird.
