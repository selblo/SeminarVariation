...
\]
Somit erhalten wir die einfache Lagrange-Funktion für eine Oberflächenwasserwelle in seichten Gewässer:
\[
L(\bm{x},z,t,\eta,\phi,\bm{u}, v, \bm{\mu},\upsilon,\phi_t,\phi_x,\phi_y,\phi_z)
=
\]
...
%Variation nach eta
	...
	\left(\rho\bar{\phi}\frac{\partial \eta(\bm{x}, t)}{\partial t}
	-
	\frac{1}{2} \rho g \eta(\bm{x}, t)^2
	+
	\rho(\eta(\bm{x}, t) + h)
...
Durch Vereinfachung ergibt nämlich diese die nicht viskose Burgers-Gleichung sowie Ähnlichkeiten zur Lösung der linearen Wellengleichung im eindimensionalen Fall.
